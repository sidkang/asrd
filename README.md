...
- Realtime WebSocket transcription: `WS /v1/realtime`
...
Run once without installing from a pinned commit:

```bash
uvx --from git+https://github.com/sidkang/asrd.git@<commit-sha> asrd --help
```

Install as a persistent user tool:

```bash
uv tool install git+https://github.com/sidkang/asrd.git@<commit-sha>
asrd --help
```

Install from `master` instead if you intentionally want the latest changes:
...
`/health` is intentionally unauthenticated for local monitoring. Do not bind the service to an untrusted network.
