...
REALTIME_FINAL_WAIT_TIMEOUT_BASE_SEC = 3.0
REALTIME_FINAL_WAIT_TIMEOUT_PER_AUDIO_SEC = 0.20
...
CORRECTION_WORKER_PIDFILE_PREFIX = "asrd-correction-worker"
...
class State:
    ...
    ws_correction_cancel_queue: mp.Queue | None = None
    ws_correction_pidfile: Path | None = None
    ws_correction_responses: dict[str, dict[str, Any]] = field(default_factory=dict)
    ws_correction_cancelled_ids: set[str] = field(default_factory=set)
...
def create_app(config: Config) -> FastAPI:
    ...
    async def health() -> dict:
        ...
        return {
            ...
            "realtime_final_wait_timeout_policy": (
                f"base={REALTIME_FINAL_WAIT_TIMEOUT_BASE_SEC:.0f}s + "
                f"{REALTIME_FINAL_WAIT_TIMEOUT_PER_AUDIO_SEC:.2f}x estimated correction window"
            ),
    ...
    async def realtime(websocket: WebSocket):
        ...
        try:
            while True:
                ...
                if action == "run-task":
                    ...
                    _, final_timed_out = await ws_update_final_correction_with_deadline(
                        ...
                        language=language,
                        prompt=prompt,
                        timeout_sec=realtime_final_wait_timeout_sec(total_audio_sec, rolling_correction.stable_until_sec),
                        client=client,
                        protocol="aliyun-fun",
                    )
            ...
            final_requested = True
            discard_preview_tasks()
            if state.ws_correction_lock is not None and rolling_correction.pending_request_ids:
                async with state.ws_correction_lock:
                    cancel_pending_ws_corrections(state, rolling_correction, reason="ws-close")
...
def realtime_final_wait_timeout_sec(audio_sec: float, stable_until_sec: float) -> float:
    audio_sec = max(0.0, audio_sec)
    stable_until_sec = min(max(0.0, stable_until_sec), audio_sec)
    remaining_sec = max(0.0, audio_sec - stable_until_sec)
    if stable_until_sec <= 0.1:
        estimated_correction_window_sec = min(audio_sec, FINAL_CATCHUP_MAX_WINDOW_SEC)
    else:
        left_context_sec = max(FINAL_CATCHUP_LEFT_CONTEXT_SEC, ROLLING_CORRECTION_COMMIT_POSITION_SEC)
        estimated_correction_window_sec = min(audio_sec, remaining_sec + left_context_sec)
    return REALTIME_FINAL_WAIT_TIMEOUT_BASE_SEC + estimated_correction_window_sec * REALTIME_FINAL_WAIT_TIMEOUT_PER_AUDIO_SEC
    ...
    async with state.ws_correction_lock:
        ...
        while True:
            ...
            if response is not None:
                ...
                state.last_used_at = time.monotonic()
                return response
            if not check_and_cleanup_ws_correction_worker(state):
...
def cancel_pending_ws_corrections(state: State, correction: WsRollingCorrectionSession, *, reason: str) -> None:
    ...
    if state.ws_correction_cancel_queue is not None:
        try:
            state.ws_correction_cancel_queue.put({"type": "cancel", "ids": pending})
        except Exception:
            pass
    state.ws_correction_cancelled_ids.update(pending)
...
def ws_collect_correction_worker_responses(state: State) -> None:
    ...
    while True:
        ...
        if response_id:
            if response_id in state.ws_correction_cancelled_ids:
                state.ws_correction_cancelled_ids.discard(response_id)
                continue
...
def correction_worker_pidfile(config: Config) -> Path:
    return Path(tempfile.gettempdir()) / f"{CORRECTION_WORKER_PIDFILE_PREFIX}-{config.port}.pid"


def check_and_cleanup_ws_correction_worker(state: State) -> bool:
    process = state.ws_correction_process
    if process is None:
        return False
    if process.is_alive():
        return True
    if state.ws_correction_pidfile is not None:
        remove_pidfile(state.ws_correction_pidfile, process.pid)
    state.ws_correction_process = None
    state.ws_correction_request_queue = None
    state.ws_correction_response_queue = None
    state.ws_correction_cancel_queue = None
    state.ws_correction_pidfile = None
    return False


def ensure_ws_correction_worker_started(state: State, config: Config) -> None:
    if (
        check_and_cleanup_ws_correction_worker(state)
        and state.ws_correction_request_queue is not None
        and state.ws_correction_response_queue is not None
        and state.ws_correction_cancel_queue is not None
    ):
        return
    pidfile = correction_worker_pidfile(config)
    cleanup_stale_pidfile(pidfile, label=f"Realtime correction worker port={config.port}")
    state.ws_correction_request_queue = None
    state.ws_correction_response_queue = None
    state.ws_correction_cancel_queue = None
    state.ws_correction_process = None
    ...
    process = ctx.Process(
        ...
        args=(config, request_queue, response_queue, cancel_queue),
    ...
    state.ws_correction_cancel_queue = cancel_queue
    state.ws_correction_process = process
    state.ws_correction_pidfile = pidfile
...
def stop_ws_correction_worker(state: State, *, reason: str) -> None:
    ...
    state.ws_correction_cancel_queue = None
    state.ws_correction_pidfile = None
    state.ws_correction_responses.clear()
    state.ws_correction_cancelled_ids.clear()
...
def ws_correction_worker_main(config: Config, request_queue: mp.Queue, response_queue: mp.Queue, cancel_queue: mp.Queue) -> None:
    ...
    def drain_cancel_requests() -> None:
        while True:
            try:
            ...
            if message.get("type") != "cancel":
                continue
            for request_id in message.get("ids") or []:
                if request_id:
                    cancelled_request_ids.add(str(request_id))

    def request_cancelled(request_id: Any) -> bool:
        drain_cancel_requests()
        value = str(request_id)
        if value in cancelled_request_ids:
            cancelled_request_ids.discard(value)
            return True
        return False
...
def debug_artifact_audio_path(config: Config, *, kind: str, safe_name: str, suffix: str) -> Path:
    ...
    chmod_private(target_dir, is_dir=True)
