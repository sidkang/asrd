...
def test_realtime_final_wait_timeout_is_adaptive():
    assert server.realtime_final_wait_timeout_sec(57.5, 0.0) == pytest.approx(14.5)
    assert server.realtime_final_wait_timeout_sec(132.9, 110.0) == pytest.approx(11.58)
    assert server.realtime_final_wait_timeout_sec(8.0, 0.0) == pytest.approx(4.6)
    assert server.realtime_final_wait_timeout_sec(300.0, 0.0) == pytest.approx(15.0)


def test_correction_worker_pidfile_is_port_scoped():
    config_a = server.Config(
        host="127.0.0.1",
        port=5100,
        api_key="password",
        model="final-model",
        ...
        unload_after_sec=0,
        debug=False,
        debug_dir="/tmp/asrd-debug",
    )
    config_b = dataclasses.replace(config_a, port=5101)

    assert server.correction_worker_pidfile(config_a) != server.correction_worker_pidfile(config_b)
    assert str(config_a.port) in server.correction_worker_pidfile(config_a).name


def test_cancel_pending_ws_corrections_marks_late_responses_without_stopping_worker():
    class DummyCancelQueue:
        def __init__(self):
            self.messages = []
    ...
    state = server.State()
    cancel_queue = DummyCancelQueue()
    state.ws_correction_cancel_queue = cancel_queue  # type: ignore[assignment]
    state.ws_correction_responses["req-1"] = {"id": "req-1", "ok": True}
    correction = server.WsRollingCorrectionSession()
    correction.pending_request_ids.add("req-1")
    correction.pending_requests["req-1"] = {"reason": "rolling"}

    server.cancel_pending_ws_corrections(state, correction, reason="test")

    assert cancel_queue.messages == [{"type": "cancel", "ids": ["req-1"]}]
    assert "req-1" in state.ws_correction_cancelled_ids
    assert "req-1" not in state.ws_correction_responses
    assert not correction.pending_request_ids
    assert not correction.pending_requests
